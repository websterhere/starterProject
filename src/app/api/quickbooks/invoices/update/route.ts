...
export async function POST(req: NextRequest) {
  ...
  try {
    const updated = await new Promise((resolve, reject) => {
      qbo.updateInvoice(invoiceData, (err: unknown, data: unknown) => {
        if (err) reject(err);
        else resolve(data);
      });
    });
    ...
    return NextResponse.json({ error: 'Failed to update invoice', details: error instanceof Error ? error.message : error }, { status: 500 });
  }
} 
...
export async function POST(req: NextRequest) {
  ...
  const { idOrEntity } = await req.json();

  try {
    const deleted = await new Promise((resolve, reject) => {
      qbo.deleteInvoice(idOrEntity, (err: unknown, data: unknown) => {
        if (err) reject(err);
        else resolve(data);
      });
    });
    ...
    return NextResponse.json({ error: 'Failed to delete invoice', details: error instanceof Error ? error.message : error }, { status: 500 });
  }
} 
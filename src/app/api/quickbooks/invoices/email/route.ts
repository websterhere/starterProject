...
export async function POST(req: NextRequest) {
  ...
  const { id, sendTo } = await req.json();

  try {
    const emailed = await new Promise((resolve, reject) => {
      qbo.sendInvoicePdf(id, sendTo, (err: unknown, data: unknown) => {
        if (err) reject(err);
        else resolve(data);
      });
    });
    ...
    return NextResponse.json({ error: 'Failed to email invoice PDF', details: error instanceof Error ? error.message : error }, { status: 500 });
  }
} 